...
internal class ForgotPasswordUsecaseRemote<T: Codable>: IForgotPasswordUsecase {
    ...
    func forgotPassword(request: ForgotPasswordModel.Request) {
        ...
        let accessToken:String = PlatformUtility.serviceId!
                     let requestAPI = APIRequest<ForgotPasswordModel.Request>(method: .post, url: APIConstants.ServiceNames.forgotPassword, headers: [APIConstants.HTTPStrings.contentTypeHeader: APIConstants.HTTPStrings.contentTypeJSON, APIConstants.HTTPStrings.authorizationHeader: accessToken], params: nil, paramsEncoding: .json, multiPartImageDict: nil, mutliParamsDict: nil)
