...
extension VehiclesRouter: IRouter {
    func perform(viewModel: Any) {
        //push viewcontroller
        if let viewModel = viewModel as? VehiclesListModel.PresentionModel {
        ...
        } else if let viewModel = viewModel as? SearchVehiclesModel.PresentionModel {
            if let route = viewModel.route {
                switch route.navigation {
