...
extension UpdateAddressRouter: IRouter {
    func perform(viewModel: Any) {
        //push viewcontroller
        if let viewModel = viewModel as? UpdateAddressModel.PresentionModel {
        ...
        } else if let viewModel = viewModel as? UpdateAddressModel.PresentionModel {
            if let route = viewModel.route {
                switch route.navigation {
