...
class ResetPasswordPresenter {
    var viewController: IResetPasswordViewable?
}

extension ResetPasswordPresenter: IResponseHandler {
    ...
    func onSuccess<T>(response: T) {
        ...
        if let responseModel = response as? ResetPasswordModel.Response {
            ...
            if responseModel.statusCode == "0" {
                viewController?.sendEmailLinkSuccess(viewModel: viewModel)
            } else {
                viewModel.message = responseModel.message
                viewController?.sendEmailLinkFailed(viewModel: viewModel)
            }
        }
    }
